...
The methodology emphasizes a step-by-step, detailed approach to understand the data intricacies and build a robust predictive model. The final model achieved a Root Mean Squared Logarithmic Error (RMSLE) of 0.13 on the test set, demonstrating strong predictive performance.
...
*   **Distribution:** The `SalePrice` showed significant positive (right) skewness...
    ![Distribution and Q-Q Plot of Original SalePrice](images/original_saleprice_distribution.png)
*   **Transformation:** A natural logarithm transformation (`np.log(SalePrice)`) was applied. The transformed target, `SalePrice_Log`, exhibited a much more symmetrical, normal-like distribution (skewness ~0.12, kurtosis ~0.81). This transformed target was used for modeling.
    ![Distribution and Q-Q Plot of Log-Transformed SalePrice](images/log_saleprice_distribution.png)
...
*   **Scatter Plots of Key Predictors vs. Target:**
    ...
    *   These plots also helped identify potential outliers, such as a few properties with very large `GrLivArea` but unexpectedly moderate `SalePrice_Log`.
    ...
    ![Scatter Plots of Key Numerical Features vs. Log(SalePrice)](images/numerical_features_scatter_plot.png)
...
![Box Plots of Key Categorical Features vs. Log(SalePrice)](images/categorical_features_box.png)
